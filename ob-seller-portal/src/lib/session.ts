 ...
 * des sessions que pour le staff OB). Un cookie httpOnly signé porte
...
export async function verifySessionToken(token: string): Promise<SellerSessionUser | null> {
  try {
    const { payload } = await jwtVerify(token, getJwtSecret(), { issuer: "ob-seller-portal" });
    if (
      !payload.sub ||
