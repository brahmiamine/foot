...
- **`sellerPortal`** : renommage (`ob-seller-portal` → `sellerPortal`), généralisation des textes d'interface, et scoping multi-clubs réel faits (`sp_sellers.club_id`/`sp_product_categories.club_id`, choix du club à l'inscription, `clubId` porté par la session, catégories filtrées par club). Voir `sellerPortal/README.md` § « Portée V1 ». Reste hors périmètre : brancher `ClubBranding` (nom/logo/couleurs affichés) sur le club du vendeur connecté.
