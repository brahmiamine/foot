...
| [`sellerPortal`](./sellerPortal) | Portail vendeur générique du marketplace d'un club : catalogue produits/variantes, stock, commandes, retours, payouts (lecture seule), notifications — les vendeurs sont des comptes indépendants du SSO club. Anciennement `ob-seller-portal`. | 3006 | Cookie de session propre (`SP_JWT_SECRET`), indépendant du SSO |
...
`sellerPortal` tourne désormais sur le port fixe `3006` (`package.json` : `next dev --webpack -p 3006`) — ne collisionne plus ni avec `sso` (3004) ni avec `arbinote` (3000, port par défaut de Next qu'il occupait implicitement avant ce correctif).
