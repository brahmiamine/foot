...
| `tickets.platform.tn` | Billetterie générique — app séparée [`billetterie`](./billetterie) (voir tableau `## Applications` et son README pour la portée V1 : achat mock). Admin des catégories/règles de vente dans `teamManager` (`/admin/billetterie`). `ob/espace-membre/billets` reste un écran d'attente qui pointe vers cette billetterie générique plutôt que de la réimplémenter dans l'app custom (écart restant : le lien n'est pas encore branché, voir « Écarts connus » ci-dessous) |
