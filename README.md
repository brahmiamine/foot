...
| [`sellerPortal`](./sellerPortal) | Portail vendeur générique du marketplace d'un club : catalogue produits/variantes, stock, commandes, retours, payouts (lecture seule), notifications — les vendeurs sont des comptes indépendants du SSO club. Anciennement `ob-seller-portal`. | 3007 | Cookie de session propre (`SP_JWT_SECRET`), indépendant du SSO |
...
`sellerPortal` tourne désormais sur le port fixe `3007` (`package.json` : `next dev --webpack -p 3007`) — ne collisionne plus ni avec `arbinote` (3000, port par défaut de Next qu'il occupait implicitement avant ce correctif) ni avec `payment-api` (3006, déjà supposé par `billetterie/.env.example` et `teamManager/.env.example` avant ce correctif — `sellerPortal` avait initialement pris ce même port par erreur, corrigé ici plutôt que de déplacer `payment-api`).
