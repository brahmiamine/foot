...
export class AvailabilityService {
  ...
  async listMine(userId: string): Promise<RefereeUnavailability[]> {
    return (await this.repository()).find({ where: { userId }, order: { startDate: "DESC", createdAt: "DESC" } });
  }

  async createMine(userId: string, startDate: string, endDate: string, reason?: string | null): Promise<RefereeUnavailability> {
    const validationError = validateDateRange(startDate, endDate);
    if (validationError) throw new AvailabilityError(validationError);
    const repo = await this.repository();
    const overlapping = await repo.createQueryBuilder("period")
      .where("period.user_id = :userId", { userId })
      .andWhere("period.cancelled_at IS NULL")
      .andWhere("NOT (:endDate < period.start_date OR :startDate > period.end_date)", { startDate, endDate })
      .getOne();
    if (overlapping) throw new AvailabilityError("Cette période chevauche une indisponibilité existante");
    return repo.save(repo.create({ userId, startDate, endDate, reason: reason?.trim().slice(0, 500) || null }));
  }
