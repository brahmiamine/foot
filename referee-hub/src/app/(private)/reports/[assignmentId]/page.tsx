...
export default async function ReportEditorPage({ params, searchParams }: { params: Promise<{ assignmentId: string }>; searchParams: Promise<{ success?: string; error?: string }> }) {
  ...
  return <>
    ...
    <PageHeading title={locale === "ar" ? "تقرير تكميلي" : "Rapport complémentaire"} description={`${fixture} · ${formatDate(match.date, locale)} · ${roleLabel(assignment.role, locale)}`} />
    ...
    <section className="panel-card report-editor">
      ...
      <form action={saveReportAction} className="portal-form">
        ...
        <label className="full-field">{locale === "ar" ? "محتوى التقرير" : "Contenu du rapport"}<textarea name="content" rows={14} required defaultValue={report?.content ?? ""} disabled={submitted} placeholder={locale === "ar" ? "صف الوقائع أو الحوادث أو الملاحظات التكميلية…" : "Décrivez les faits, incidents ou observations complémentaires…"} /></label>
