...
import { RefereeReportPolicyService } from "@/services/RefereeReportPolicyService";

/** REF-004 — lecture/écriture de la policy PLATFORM depuis un service consommateur (ex: federation-hub). */
export async function GET(request: NextRequest) {
  const authError = ensureRefereeServiceAuth(request);
  if (authError) return authError;
  const resolved = await new RefereeReportPolicyService().resolveExplained();
...
export async function POST(request: NextRequest) {
  ...
  try {
    const saved = await new RefereeReportPolicyService().upsert({
      values: body.values,
