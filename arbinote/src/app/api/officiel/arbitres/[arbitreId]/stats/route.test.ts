...
vi.mock('@/lib/adminAuth', () => ({
  getOfficialEvalSession: (...args: unknown[]) => mockSession(...args),
}))
vi.mock('@/lib/refereeOfficialAnalytics', () => ({
  getRefereeOfficialAnalytics: (...args: unknown[]) => mockAnalytics(...args),
}))
...
describe('GET /api/officiel/arbitres/:arbitreId/stats', () => {
  it('rejects requests without an official evaluation session', async () => {
    mockSession.mockResolvedValue(null)
    const response = await GET(
      new NextRequest('http://localhost/api/officiel/arbitres/ref-1/stats'),
  ...
  it('returns validated official trend and observer comparison', async () => {
    mockSession.mockResolvedValue({ id: 'fed-admin', role: 'FEDERATION_ADMIN' })
