...
export function useAdminVotes() {
  ...
  const loadFilterOptions = useCallback(async () => {
    try {
      const res = await fetch('/api/admin/votes?getFilters=true')
  ...
  const saveEdit = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/votes/single/${editModal.vote.id}`, {
        method: 'PUT',
  ...
  const deleteVote = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/votes/single/${id}`, {
        method: 'DELETE',
      })
