...
export default async function NotificationsPage() {
  ...
  return (
    <div style={{ display: "grid", gap: "1rem" }}>
      ...
      {notifications.map((n) => (
        <Card key={n.id} style={{ background: n.readAt ? "var(--mh-surface)" : "var(--mh-surface-alt)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
            <div>
              ...
              <div style={{ fontSize: "0.85rem", color: "var(--mh-text-muted)", marginTop: 4 }}>{n.body}</div>
              <div style={{ fontSize: "0.75rem", color: "var(--mh-text-subtle)", marginTop: 6 }}>{formatDateTime(n.createdAt)}</div>
            </div>
