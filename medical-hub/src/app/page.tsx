  ...
  return (
    <div style={{ display: "grid", gap: "1.25rem" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: "0.9rem" }}>
        <StatCard label={t("dashboard.stats.roster")} value={String(summary.totalPlayers)} tone="primary" />
        <StatCard label={t("dashboard.stats.available")} value={String(summary.available)} tone="success" />
        <StatCard label={t("dashboard.stats.ongoing")} value={String(summary.ongoing)} tone="danger" />
        <StatCard label={t("dashboard.stats.recovering")} value={String(summary.recovering)} tone="warning" />
      </div>

      <Card>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <div style={{ fontSize: "0.8rem", color: "var(--mh-text-muted)", fontWeight: 600 }}>
        ...
        {alerts.length === 0 ? (
          ...
          <div style={{ display: "grid", gap: 8 }}>
            {alerts.slice(0, 5).map(({ injury, player, kind, daysDiff }) => {
              const message = getAlertMessage(kind, daysDiff);
              return (
                <div key={injury.id} style={{ display: "flex", justifyContent: "space-between", fontSize: "0.85rem" }}>
                  <span>{player ? `${player.firstNameFr} ${player.lastNameFr}` : injury.playerId} — {injury.zone}</span>
                  <Badge label={t(message.key, message.values)} tone={kind === "OVERDUE" ? "danger" : "warning"} />
                </div>
              );
            })}
          </div>
            ...
            {unavailable.slice(0, 5).map(({ injury, player }) => (
              <div key={injury.id} style={{ display: "flex", justifyContent: "space-between", fontSize: "0.85rem" }}>
                ...
                <span style={{ color: "var(--mh-text-muted)" }}>
                  {t("dashboard.unavailable.expectedReturn", {
                    date: injury.expectedReturnDate ? formatDate(injury.expectedReturnDate, locale) : "—",
                  })}
                </span>
              </div>
            ))}
