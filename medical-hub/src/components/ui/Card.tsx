...
export function StatCard({
  ...
  return (
    <Card>
      <div style={{ fontSize: "0.8rem", color: "var(--mh-text-muted)", fontWeight: 600, marginBottom: 8 }}>
        {label}
      </div>
      <div style={{ fontSize: "1.6rem", fontWeight: 700, color: accent }}>{value}</div>
      {hint && <div style={{ fontSize: "0.78rem", color: "var(--mh-text-subtle)", marginTop: 4 }}>{hint}</div>}
    </Card>
  );
}
