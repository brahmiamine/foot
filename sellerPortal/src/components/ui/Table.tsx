...
export function Thead({ children }: { children: ReactNode }) {
  return (
    <thead>
      <tr style={{ textAlign: "left", borderBottom: "1px solid var(--sp-border)" }}>{children}</tr>
    </thead>
  );
}
...
export function Td({ children, style }: { children: ReactNode; style?: React.CSSProperties }) {
  return (
    <td style={{ padding: "0.75rem 0.9rem", borderBottom: "1px solid var(--sp-border)", verticalAlign: "middle", ...style }}>
      {children}
    </td>
  );
}
