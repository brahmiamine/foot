...
beforeEach(() => {
  process.env.MARKETPLACE_API_URL = "http://marketplace-api.internal";
...
/** TS-35 — transitions produits déléguées à marketplace-api (TS-04). */
describe("marketplaceApiClient", () => {
  ...
  it("submit: calls the internal endpoint with sellerId, the service API key, and no body", async () => {
    ...
    expect(url).toBe(
      "http://marketplace-api.internal/internal/products/product-1/submit?sellerId=seller-1",
    );
  ...
  it("withdraw: hits the withdraw endpoint", async () => {
    ...
    expect(fetchMock.mock.calls[0][0]).toBe(
      "http://marketplace-api.internal/internal/products/product-1/withdraw?sellerId=seller-1",
    );
  });
  ...
  it("toggleActive: hits the toggle-active endpoint", async () => {
    ...
    expect(fetchMock.mock.calls[0][0]).toBe(
      "http://marketplace-api.internal/internal/products/product-1/toggle-active?sellerId=seller-1",
    );
  });
