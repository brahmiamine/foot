/**
 * Client HTTP serveur-à-serveur vers marketplace-api (voir
 * ../../marketplace-api/README.md), pour les écritures produit (create,
 * update, delete, submit, withdraw) — voir avancement.md, TS-04.
 * sellerPortal a déjà authentifié le vendeur via sa propre session
 * (src/lib/session.ts, SP_JWT_SECRET) : ces routes internal/* de
 * marketplace-api font confiance à `sellerId` passé explicitement plutôt
 * que d'exiger un second login vendeur (marketplace-api a son propre JWT
 * self-service, SELLER_JWT_SECRET, non interchangeable avec celui-ci).
 * Les lectures produit restent en TypeORM direct côté sellerPortal (même
...
async function apiFetch<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    throw new MarketplaceApiError((body as { message?: string })?.message ?? `marketplace-api ${response.status}`, response.status);
  }
...
/** Bascule isActive, sans restriction de statut (voir marketplace-api ProductsService.toggleActive). */
