...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!user || !user.isActive || !user.mfaEnabled || !user.mfaSecret) {
      ...
      await logSecurityEvent({ eventType: "MFA_VERIFY_FAILED", userId, ip: clientIP, detail: "Compte introuvable ou MFA non active" });
    ...
    if (!valid) {
      recordFailedLoginAttempt(clientIP);
      await logSecurityEvent({ eventType: "MFA_VERIFY_FAILED", userId: user.id, email: user.email, ip: clientIP });
      return NextResponse.json({ error: "Code invalide" }, { status: 401 });
    }
