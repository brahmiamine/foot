...
| Disponibilités personnelles des arbitres | `referee_unavailabilities` | `referee-hub` (écriture personnelle, `user_id` issu du JWT) | `match-operations` (blocage d'affectation) ; `federation-hub` (aide à la désignation) |
| Rapports complémentaires d'arbitres | `referee_match_reports` | `referee-hub` (brouillon/envoi personnel après match terminé) | `federation-hub` (lecture privée, scopée par fédération/ligue) |
...
| Discipline fédérale avancée (migration-v2.md, P1-007) | `disciplinary_cases`, `disciplinary_case_evidence`, `disciplinary_case_hearings`, `disciplinary_case_decisions`, `disciplinary_case_events` | `federation-hub` (ouverture, instruction, audiences, décisions) | — (pas d'UI `club-hub` dédiée ; une décision peut créer une ligne `club_sanctions` liée, source de vérité inchangée) |
| Appels (migration-v2.md, P1-008) | `appeals`, `appeal_documents`, `appeal_events` | `club-hub` (dépôt d'un appel par le club requérant) **et** `federation-hub` (instruction et décision) — mêmes tables partagées | — |
| Federal Operations V3 : commissions, assurances, subventions, droits média, formation/solidarité, conformité documentaire, sélections nationales (federation-hub/doc/federal-operations-v3.md) | `federal_commissions`, `federal_commission_members`, `federal_commission_sessions`, `federal_commission_attendance`, `federal_commission_decisions`, `club_insurance_policies`, `person_insurance_policies`, `federation_grants`, `grant_applications`, `grant_payments`, `broadcasting_revenue_distributions`, `broadcasting_revenue_allocations`, `training_compensation_cases`, `training_compensation_beneficiaries`, `solidarity_contributions`, `solidarity_allocations`, `regulatory_document_requirements`, `regulatory_document_submissions`, `national_team*`, `federal_operation_audit` | `federation-hub` (seul écrivain, scopé plateforme/fédération/ligue — voir `federalOperationsCommon.ts`) | — |
| Regulatory Policy Center — politiques réglementaires hiérarchiques (FED-001..004, platform-governance-roadmap.md §N) | `federal_regulatory_policies` | `federation-hub` (résolution via `@foot/domain-contracts/policy`, versionnée et auditée — voir `regulatoryPolicyCenter.ts`) | — |
| SLA réglementaires (FED-010, platform-governance-roadmap.md §N) | `regulatory_sla_policies` | `federation-hub` (seuils par domaine/fédération, file overdue calculée à la volée — voir `regulatorySla.ts`) | — |
| Mode de migration des permissions réglementaires (FED-011, platform-governance-roadmap.md §N) | `regulatory_permission_mode_settings`, `regulatory_permission_warnings` | `federation-hub` (mode global LEGACY/WARN/ENFORCE, journal des refus WARN — voir `regulatoryPermissions.ts`) | — |
