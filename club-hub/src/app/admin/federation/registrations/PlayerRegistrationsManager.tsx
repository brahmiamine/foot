...
interface Registration { id: string; playerId: string; seasonId: string; licenseId: string; contractId: string | null; status: RegistrationStatus; eligibilityStatus: EligibilityStatus; registeredAt: string | null; rejectionReason: string | null; }
...
interface Contract { id: string; playerId: string; seasonId: string; contractType: string; endDate: string; }
...
const TEXT = {
  fr: { title: "Inscriptions joueurs", subtitle: "Inscrivez les joueurs licenciés à une compétition-saison et suivez leur éligibilité.", new: "Nouvelle inscription", player: "Joueur", season: "Compétition · saison", license: "Licence active", contract: "Contrat homologué", status: "Statut", eligibility: "Éligibilité", open: "Ouvrir", create: "Créer", submit: "Soumettre", reopen: "Corriger le dossier", loading: "Chargement…", empty: "Aucune inscription.", history: "Historique", noHistory: "Aucun événement.", decision: "Décision réglementaire", close: "Fermer", selectPlayer: "Sélectionnez d’abord un joueur", noLicense: "Aucune licence PLAYER active ne correspond à ce joueur et à cette saison.", contractRequired: "Cette compétition exige un contrat joueur homologué." },
  ar: { title: "تسجيلات اللاعبين", subtitle: "سجّل اللاعبين أصحاب التراخيص في مسابقة وموسم وتابع أهليتهم.", new: "تسجيل جديد", player: "اللاعب", season: "المسابقة · الموسم", license: "الترخيص الساري", contract: "العقد المصادق عليه", status: "الحالة", eligibility: "الأهلية", open: "فتح", create: "إنشاء", submit: "إيداع", reopen: "تصحيح الملف", loading: "جار التحميل…", empty: "لا يوجد تسجيل.", history: "السجل", noHistory: "لا يوجد حدث.", decision: "القرار التنظيمي", close: "إغلاق", selectPlayer: "اختر اللاعب أولا", noLicense: "لا يوجد ترخيص PLAYER ساري لهذا اللاعب في هذا الموسم.", contractRequired: "تتطلب هذه المسابقة عقد لاعب مصادقا عليه." },
};
...
export default function PlayerRegistrationsManager() {
  ...
  const [form, setForm] = useState({ playerId: "", seasonId: "", licenseId: "", contractId: "" });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRegistrations(data.registrations); setPlayers(data.candidates.players); setLicenses(data.candidates.licenses); setContracts(data.candidates.contracts); setSeasons(data.seasons); setCanManage(data.canManage);
  ...
  const matchingContracts = useMemo(() => contracts.filter((contract) => (!form.playerId || contract.playerId === form.playerId) && (!form.seasonId || contract.seasonId === form.seasonId)), [contracts, form.playerId, form.seasonId]);
  const requiresContract = seasons.find((season) => season.id === form.seasonId)?.requiresPlayerContract === true;
  ...
  return <div className="container-fluid px-0 d-flex flex-column gap-4">
    ...
    {canManage && <div className="card border-0 shadow-sm"><div className="card-body"><h2 className="h6">{text.new}</h2><div className="row g-2">
      <div className="col-md-3"><label className="form-label">{text.player}</label><select className="form-select" value={form.playerId} onChange={(event) => setForm({ playerId: event.target.value, seasonId: "", licenseId: "", contractId: "" })}><option value="">—</option>{players.map((player) => <option key={player.id} value={player.id}>{player.name} · {player.category}</option>)}</select></div>
      <div className="col-md-3"><label className="form-label">{text.season}</label><select className="form-select" disabled={!form.playerId} value={form.seasonId} onChange={(event) => setForm({ ...form, seasonId: event.target.value, licenseId: "", contractId: "" })}><option value="">{form.playerId ? "—" : text.selectPlayer}</option>{availableSeasons.map((season) => <option key={season.id} value={season.id}>{season.nom} · {season.competitionType}</option>)}</select></div>
      <div className="col-md-2"><label className="form-label">{text.license}</label><select className="form-select" disabled={!form.seasonId} value={form.licenseId} onChange={(event) => setForm({ ...form, licenseId: event.target.value })}><option value="">—</option>{matchingLicenses.map((license) => <option key={license.id} value={license.id}>{license.number ?? license.id}</option>)}</select></div>
      <div className="col-md-3"><label className="form-label">{text.contract}{requiresContract ? " *" : ""}</label><select className="form-select" disabled={!form.seasonId} value={form.contractId} onChange={(event) => setForm({ ...form, contractId: event.target.value })}><option value="">—</option>{matchingContracts.map((contract) => <option key={contract.id} value={contract.id}>{contract.contractType} · {new Date(contract.endDate).toLocaleDateString()}</option>)}</select></div>
      <div className="col-md-1 d-flex align-items-end"><button disabled={saving || !form.licenseId || (requiresContract && !form.contractId)} className="btn btn-primary w-100" onClick={() => void create()}>{text.create}</button></div>
    </div>{form.playerId && form.seasonId && matchingLicenses.length === 0 && <p className="small text-warning mt-2 mb-0">{text.noLicense}</p>}{requiresContract && matchingContracts.length === 0 && <p className="small text-warning mt-2 mb-0">{text.contractRequired}</p>}</div></div>}
    ...
    {selected && <div className="card border-primary shadow-sm"><div className="card-header bg-transparent d-flex justify-content-between"><div><strong>{playerNames.get(selected.registration.playerId) ?? selected.registration.playerId}</strong> <span className={`badge ms-2 ${BADGES[selected.registration.status]}`}>{selected.registration.status}</span> <span className={`badge ms-1 ${ELIGIBILITY_BADGES[selected.registration.eligibilityStatus]}`}>{selected.registration.eligibilityStatus}</span></div><button className="btn-close" aria-label={text.close} onClick={() => setSelected(null)} /></div><div className="card-body d-flex flex-column gap-3">
      <div><strong>{text.season}:</strong> {seasonNames.get(selected.registration.seasonId) ?? selected.registration.seasonId}<br /><strong>{text.contract}:</strong> {selected.registration.contractId ?? "—"}</div>
