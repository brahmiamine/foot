 ...
 * paiement PAID (voir payments/src/webhooks, et
 * ticketing/src/app/api/payments/webhook/route.ts pour la même route côté
 * ticketing). Le corps n'est jamais source de vérité : on vérifie
 * seulement la signature pour authentifier l'appelant, puis on déclenche
 * reconcileOrderPayment qui relit GET /payments/:id auprès de payments
 * avant de marquer la commande PAID.
 *
 * TASK-P0-005 : payments retente la livraison en cas d'échec réseau/
 * timeout (voir payments/src/webhooks/webhook-dispatch.service.ts), le
 ...
 * ré-exécuter le traitement métier (même pattern que ticketing).
 */
