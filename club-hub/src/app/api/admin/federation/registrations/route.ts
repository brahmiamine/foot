...
export async function GET() {
  ...
  try {
    ...
    const [registrations, candidates, seasons] = await Promise.all([
      listPlayerRegistrationsForClub(session.user.teamId, source),
      listRegistrationCandidates(session.user.teamId, source),
      source.query("SELECT id, nom, type_competition AS competitionType FROM saisons ORDER BY date_debut DESC, nom DESC") as Promise<Array<{ id: string; nom: string; competitionType: string }>>,
    ]);
...
export async function POST(request: NextRequest) {
  ...
  try {
    const body = await request.json() as { playerId?: string; seasonId?: string; licenseId?: string };
    ...
    const registration = await createPlayerRegistrationForClub(session.user.teamId, { playerId: body.playerId, seasonId: body.seasonId, licenseId: body.licenseId }, actor(request, session.user));
