...
function getConfig(): { baseUrl: string; apiKey: string } {
  ...
  if (!baseUrl || !apiKey) {
    throw new Error("MARKETPLACE_API_URL et MARKETPLACE_API_KEY doivent être configurés pour la marketplace.");
  }
...
async function marketplaceRequest<T>(clubId: string, operation: () => Promise<T>): Promise<T> {
  await new ClubFeatureSettingsService().assertEnabled(clubId, "MARKETPLACE");
  return operation();
}

export const listSellersForClub = (clubId: string) =>
  marketplaceRequest(clubId, () => apiFetch<MarketplaceSeller[]>(`/sellers${buildQuery({ clubId })}`));
export const listCategoriesForClub = (clubId: string) =>
  marketplaceRequest(clubId, () => apiFetch<MarketplaceCategory[]>(`/categories${buildQuery({ clubId })}`));
export const listSellerApplicationsForClub = (clubId: string, status?: string) =>
  marketplaceRequest(clubId, () => apiFetch<MarketplaceSellerApplication[]>(`/seller-applications${buildQuery({ clubId, status })}`));
export const getMarketplaceSettings = (clubId: string) =>
  marketplaceRequest(clubId, () => apiFetch<MarketplaceSettings>(`/marketplace-settings${buildQuery({ clubId })}`));
export const updateMarketplaceSettings = (
  clubId: string,
  actorId: string,
  settings: Omit<MarketplaceSettings, "clubId" | "updatedBy">,
) => marketplaceRequest(clubId, () =>
  apiFetch<MarketplaceSettings>(`/marketplace-settings${buildQuery({ clubId })}`, {
    method: "PATCH",
    body: JSON.stringify({ ...settings, actorId }),
  }),
);
export const updateSellerStatus = (id: string, clubId: string, status: string, reason?: string) =>
  marketplaceRequest(clubId, () => apiFetch<MarketplaceSeller>(`/sellers/${id}/status${buildQuery({ clubId })}`, {
    method: "PATCH",
    body: JSON.stringify({ status, reason }),
  }));
export const updateSellerCommission = (id: string, clubId: string, commissionRate: number) =>
  marketplaceRequest(clubId, () => apiFetch<MarketplaceSeller>(`/sellers/${id}/commission${buildQuery({ clubId })}`, {
    method: "PATCH",
...
async function applicationTransition(
  ...
  return marketplaceRequest(clubId, () =>
    apiFetch<MarketplaceSellerApplication | { application: MarketplaceSellerApplication; seller: MarketplaceSeller }>(
      `/seller-applications/${id}/${action}${buildQuery({ clubId })}`,
      { method: "POST", body: JSON.stringify({ actorId, reason }) },
    ),
  );
}
...
export const listModerationProducts = (clubId: string, filters: ModerationProductFilters) =>
  marketplaceRequest(clubId, () => apiFetch<MarketplaceProduct[]>(`/moderation/products${buildQuery({ clubId, ...filters })}`));
...
async function moderationTransition(
  ...
  return marketplaceRequest(clubId, () =>
    apiFetch(`/moderation/products/${id}/${action}${buildQuery({ clubId })}`, {
      method: "POST",
