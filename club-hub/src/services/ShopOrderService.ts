...
// même TTL que ticketing/src/lib/tickets.ts (PENDING_RESERVATION_TTL_MS).
 ...
 * ticketing/src/lib/tickets.ts#purgeStalePendingTickets, appelée par le
 ...
 * paiement auprès de payments. Si l'initiation échoue, la réservation
 ...
 * que ticketing/src/lib/tickets.ts#purchaseTickets.
 */
...
/**
 * payments ne rappelle jamais club-hub directement pour cette route :
 * c'est à club-hub de relire GET /payments/:id (retour du payeur, ou
 ...
 * stratégie que ticketing/src/lib/tickets.ts#reconcileTicketPayment.
 *
 ...
 * payments (TASK-P0-001) plutôt que de se limiter à journaliser un
...
export async function listMyOrders(purchaserId: string): Promise<MyOrder[]> {
  ...
  // ticketing/src/lib/tickets.ts#listMyTickets : si le client n'est
