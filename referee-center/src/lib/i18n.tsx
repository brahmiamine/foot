...
export function TranslationProvider({
  ...
  const switchLocale = (newLocale: Locale) => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('referee-center-locale', nextLocale)
      document.cookie = `referee-center-locale=${nextLocale}; path=/; max-age=31536000`
    }
  }
  ...
  useEffect(() => {
    ...
    const stored = localStorage.getItem('referee-center-locale')
    if (stored && isLocale(stored)) {
      // eslint-disable-next-line react-hooks/set-state-in-effect
      setLocale((current) => (stored !== current ? stored : current))
    } else {
      localStorage.setItem('referee-center-locale', sanitizedInitialLocale)
    }
