...
Cookie JWT vendeur signé par `SP_JWT_SECRET`, distinct du SSO. Les helpers d'autorisation vérifient vendeur et club sur les ressources; marketplace dispose par ailleurs de son propre JWT vendeur lorsque activée.
...
MariaDB partagée; SMTP pour récupération de compte; client marketplace présent mais le portail conserve encore ses entités/accès locaux; branding d'équipe partagé.
...
Architecture de transition: duplication entre tables locales et marketplace, donc cette dernière n'est pas encore l'unique source de vérité. Vérifier/backfiller `club_id` sur installations existantes. Payouts en lecture, pas de déclenchement bancaire.
