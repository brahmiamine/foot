...
/**
 * Session vendeur — authentification indépendante du SSO club (identity/ n'émet
...
export async function verifySessionToken(token: string): Promise<SellerSessionUser | null> {
  try {
    const { payload } = await jwtVerify(token, getJwtSecret(), { issuer: "seller-portal" });
    if (
      !payload.sub ||
