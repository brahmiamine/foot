...
beforeEach(() => {
  process.env.NOTIFICATION_API_URL = "http://notifications.internal";
...
/** TS-36 — notification actions (relais HTTP vers notifications). */
describe("notificationApi", () => {
  it("throws 401 without ever calling notifications when there is no SSO session", async () => {
  ...
  it("relays the SSO token as Bearer auth and returns the unread count", async () => {
    ...
    expect(url).toBe("http://notifications.internal/api/notifications/unread-count");
  ...
  it("marks a single notification as read via PATCH", async () => {
    ...
    const [url, init] = fetchMock.mock.calls[0];
    expect(url).toBe("http://notifications.internal/api/notifications/notif-1/read");
