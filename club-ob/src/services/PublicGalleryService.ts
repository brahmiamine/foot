...
export class PublicGalleryService {
  async getPhotos(teamId: string, limit = 5): Promise<GalleryPhoto[]> {
    return cachePublicData(["gallery", "photos", teamId, String(limit)], async () => {
      ...
      const items = await ds.getRepository(MediaGalleryItem).find({
        where: { galleryId: In(galleries.map((gallery) => gallery.id)) },
        relations: ["mediaItem"],
      ...
      return items
        ...
        .map((item) => ({
          ...
          altFr: item.mediaItem.altTextFr ?? null,
          altAr: item.mediaItem.altTextAr ?? null,
        }));
    }, 120);
  }

  async getAllGalleries(teamId: string): Promise<GalleryWithPhotos[]> {
    return cachePublicData(["gallery", "all", teamId], async () => {
      const ds = await getDataSource();
