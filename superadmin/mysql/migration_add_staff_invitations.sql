...
-- ADMIN/OBSERVATEUR était créé "à la main" — superadmin choisissait le mot
-- de passe lui-même et le communiquait hors plateforme. Remplacé par une
-- invitation par email à usage unique : `superadmin` crée l'invitation
-- (sans mot de passe), le destinataire choisit lui-même son mot de passe en
-- l'acceptant. Table propre à `superadmin`, qui gère déjà `User` pour les
-- comptes club (voir db/OWNERSHIP.md, "Comptes et sessions" — écrit aussi
-- par superadmin pour ce cas précis, pas seulement lu).
