 ...
 * étapes (billetterie, teamManager), chacune journalisée dans
...
describe('startMatchCancellationSaga', () => {
  ...
  it('marque le dossier MANUAL_REVIEW quand une étape échoue (panne billetterie), sans bloquer l\'autre étape', async () => {
    const { startMatchCancellationSaga } = await import('./matchSaga')
    ...
    const steps = await dataSource.getRepository(MatchSagaStep).find({ where: { saga_id: sagaCase.id } })
    const billetterieStep = steps.find((s) => s.step === 'BILLETTERIE_TICKETS')
    const teamManagerStep = steps.find((s) => s.step === 'TEAMMANAGER_CONVOCATIONS')
    expect(billetterieStep?.status).toBe('FAILED')
    expect(billetterieStep?.error).toContain('billetterie unreachable')
    expect(teamManagerStep?.status).toBe('SUCCEEDED')
  });
...
describe('retryMatchSaga', () => {
  it('rejoue seulement l\'étape en échec, jamais celle déjà réussie', async () => {
    ...
    cancelMatchTickets.mockRejectedValueOnce(new Error('billetterie unreachable'))
    cancelMatchConvocations.mockResolvedValue(undefined)
    const sagaCase = await startMatchCancellationSaga('match-1', 'Terrain impraticable.', 'admin1')
    expect(sagaCase.status).toBe('MANUAL_REVIEW')
    ...
    const billetterieSteps = steps.filter((s) => s.step === 'BILLETTERIE_TICKETS')
    expect(billetterieSteps).toHaveLength(2); // attempt 1 (FAILED) + attempt 2 (SUCCEEDED) — append-only
    expect(billetterieSteps[0].attempt).toBe(1)
    expect(billetterieSteps[1].attempt).toBe(2)
  });
  ...
  it('un rejeu qui échoue à nouveau reste MANUAL_REVIEW, avec l\'erreur mise à jour', async () => {
    ...
    cancelMatchTickets.mockRejectedValue(new Error('billetterie still down'))
    const retried = await retryMatchSaga(sagaCase.id)
    ...
    expect(steps).toHaveLength(2)
    expect(steps[1].error).toContain('billetterie still down')
  });
