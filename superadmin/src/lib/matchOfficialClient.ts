/**
 * Client HTTP vers `matchsheet` pour l'affectation des officiels de match
 * (migration.md §11, Phase 4) — même pattern que matchsheetClient.ts
 ...
 * `matchsheet` (c'est aussi lui qui vérifie l'affectation avant de laisser
 * un officiel accéder à un match, voir [matchId]/layout.tsx) — superadmin
...
function getConfig(): { baseUrl: string; apiKey: string } {
  ...
  if (!baseUrl || !apiKey) {
    throw new MatchOfficialClientError('MATCHSHEET_URL/MATCHSHEET_SERVICE_API_KEY non configurés.')
  }
