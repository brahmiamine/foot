...
/**
 * Best-effort client for notification-api's POST /internal/notifications
 * (service-to-service, `x-api-key`). Never throws: a payment confirmation
 * must never fail because notification-api is unreachable or unconfigured.
 * No retry here — notification-api itself owns retry/queueing once the
 * request is accepted.
 */
@Injectable()
export class NotificationClientService {
  ...
  async notify(payload: NotifyPayload): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to reach notification-api for type=${payload.type}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
}
