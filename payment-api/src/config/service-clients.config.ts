 ...
 * internes de payment-api (initiation de paiement, lecture d'un paiement).
 * SERVICE_API_KEYS est un JSON `{ "application": "clé" }` ; chaque
 * application de l'écosystème possède sa propre clé, jamais partagée. Le
 * ServiceAuthGuard compare la clé reçue (header `x-api-key`) à ce registre
 * et en déduit l'`application` appelante. Les webhooks providers (Konnect,
 * Paymee, Flouci) ne passent jamais par cette clé : ils sont authentifiés
 * par re-vérification serveur-à-serveur / checksum, voir README § Sécurité.
 *
