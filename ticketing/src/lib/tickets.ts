 ...
 * initie le paiement auprès de payments. Si l'initiation échoue, la
 ...
 * opportunément depuis listMyTickets (payments ne rappelle jamais
 * ticketing — seuls les providers rappellent payments).
 */
export async function purchaseTickets(input: PurchaseInput): Promise<PurchaseResult> {
  ...
  // dédié côté payments) : vérifié avant toute réservation, pour ne
...
/**
 * payments ne rappelle jamais ticketing : c'est à ticketing de
...
export async function listMyTickets(purchaserId: string): Promise<MyTicket[]> {
  ...
  // "mes billets"...), on relit le statut auprès de payments ici plutôt
