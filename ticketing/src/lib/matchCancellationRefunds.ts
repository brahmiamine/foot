 ...
 * federation-hub, `matches` étant une table partagée — voir db/OWNERSHIP.md) en
 ...
 * directement par l'action d'annulation côté federation-hub (idempotencyKey
 * portée par payments, pas ici) et par le scheduler périodique
 ...
 *     synchrone déclenché par federation-hub (voir README) a échoué ou n'a
 *     jamais eu lieu (ex. ticketing indisponible au moment de
 *     l'annulation) ;
 * (2) reprendre les dossiers dont la demande a échoué et rafraîchir ceux
 *     déjà demandés mais pas encore résolus, en relisant payments —
