...
describe("cancelMatchTickets", () => {
  ...
  it("ne lève jamais quand payments est indisponible — le dossier reste PENDING_REFUND_REQUEST", async () => {
    const { cancelMatchTickets } = await import("./matchCancellationRefunds");
    await seedCancelledMatchWithTickets();
...
describe("processMatchCancellationRefunds", () => {
  it("rattrape un match CANCELLED jamais traité (federation-hub/ticketing n'ont jamais communiqué)", async () => {
