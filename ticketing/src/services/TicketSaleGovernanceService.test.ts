...
describe("TicketSaleGovernanceService", () => {
  it("requires review, forbids self-approval and schedules a future sale", async () => {
    ...
    await expect(service.assertPurchasable(offerId, new Date())).rejects.toThrow(
      "statut SCHEDULED",
    );
    const processed = await service.processDue(new Date(startsAt.getTime() + 1));
    expect(processed.opened).toBe(1);
    await expect(
      service.assertPurchasable(offerId, new Date(startsAt.getTime() + 1)),
  ...
  it("moves an OPEN sale back to review when the approved price changes", async () => {
    ...
    const rule = await dataSource
      ...
      .findOneByOrFail({ matchTicketCategoryId: offerId });
    expect(rule).toMatchObject({
      status: "READY_FOR_REVIEW",
      approvedFingerprint: null,
    });
    await expect(service.assertPurchasable(offerId)).rejects.toThrow(
      "READY_FOR_REVIEW",
    );
  });
  ...
  it("fails closed when no rule exists and prevents cross-club administration", async () => {
    ...
    await expect(
      service.getOrCreateRule(offerId, otherClubId),
    ).rejects.toThrow("n'appartient pas à votre club");
  });
