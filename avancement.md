...
~~Limite découverte en creusant le code de `payment-api`... son intégration Konnect ne transmet pas de `successUrl`/`failUrl`... à ajouter côté `payment-api`~~ — **re-vérifié le 11/08/2026 directement contre la documentation Konnect (`docs.konnect.network/docs/en/api-integration/endpoints/initiate-payment`) : ce n'est pas un champ manquant à ajouter, c'est un choix délibéré qui reste correct.** `successUrl`/`failUrl` existent bien dans l'API Konnect, mais sont documentés **dépréciés** et — point décisif — **« ignorés si un `webhook` est actif et non silencieux »**. Or `buildKonnectInitPaymentRequest` (`konnect.mapper.ts`) transmet déjà `webhook: config.webhookUrl` sans `silentWebhook: true` : les ajouter n'aurait donc aucun effet, Konnect les ignorerait silencieusement (code mort, et contraire à la règle explicite du fichier de ne jamais ajouter un champ « spéculatif »). Le rattrapage sur `/mes-billets` (`reconcileTicketPayment`) n'est donc pas un filet de secours en attendant un vrai correctif : **c'est le mécanisme de confirmation correct et définitif** compte tenu de la dépréciation de la redirection côté Konnect — le webhook reste la source de vérité temps réel, le polling au chargement de la page la couvre pour les cas où le payeur revient avant que le webhook n'ait été traité.
...
| Billetterie — confirmation retour paiement | `payment-api` → `billetterie` | ✅ Fermé (par design, re-vérifié) | `successUrl`/`failUrl` de Konnect sont documentés **dépréciés et ignorés dès qu'un `webhook` non silencieux est actif** — ce qui est déjà le cas ici. Les ajouter serait du code mort, pas un correctif. Le polling `/mes-billets` est le mécanisme de confirmation voulu, pas un pis-aller | § 3.H |
...
| `sellerPortal` — rattachement `club_id` en prod | `sellerPortal` | ✅ Fermé | `npm run backfill:club-id` (`scripts/backfill-club-id.ts`) automatise le cas non ambigu (un seul club dans `teams`) ; s'arrête proprement et liste ce qui reste à traiter à la main sinon | § « `sellerPortal`… » |
