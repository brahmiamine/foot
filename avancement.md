...
|---|---|---|
| `sso` | Invitation staff/club en 2 temps ; vrai portail compte SSO ; propagation transverse de la révocation `tokenVersion` aux apps clientes | Décider entre vérification DB/cache partagé côté apps clientes ou TTL court ; ajouter un modèle d'invitation |
...
| `billetterie` | Audience réservée auto-déclarée ; scanner stade absent ; pas de tests | Prioriser contrôle d'accès et preuve d'audience avant vente réelle sensible ; ajouter tests |
...
**Lecture rapide** : sur les 33 circuits recensés, 14 sont fermés bout-en-bout, 5 sont partiels (un filet de rattrapage ou une moitié du flux existe), 14 sont encore ouverts — dont la majorité concentrée dans `teamManager` (boutique, sponsors, RGPD, finance, espace supporter) et dans les circuits produits jamais commencés ailleurs (scanner billetterie, synchro live `superadmin`, invitation club `sso`). Aucun de ces 14 n'est bloquant pour ce que le dépôt fait déjà fonctionner ; ce sont des extensions de périmètre, pas des régressions à corriger en urgence.
