...
export class SheetService {
   ...
   * directe que `federation-hub` faisait jusqu'ici dans `ms_sheets` (et
   * `matches`) depuis `reopenMatchAdmin` : match-operations reste seul
   * propriétaire de cette transition, federation-hub passe désormais par cet
   ...
   * federation-hub/src/lib/matchOperationsClient.ts#reopenSheet) rend l'appel
   ...
   * — utile après un retry réseau côté federation-hub dont la première tentative
   * avait en fait réussi côté match-operations, seule la réponse ayant été perdue.
   */
  async reopen(
    matchId: string,
