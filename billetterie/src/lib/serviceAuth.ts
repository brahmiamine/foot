 ...
 * (clé partagée `BILLETTERIE_SERVICE_API_KEY`), jamais la session d'un
 * acheteur ou d'un vendeur. Même pattern que
 * matchsheet/src/lib/serviceAuth.ts et teamManager/src/lib/serviceAuth.ts
 * (TS-31/TASK-P0-003) — première route interne de ce type dans billetterie,
...
export function ensureServiceAuth(request: NextRequest): NextResponse | null {
  const expected = process.env.BILLETTERIE_SERVICE_API_KEY;
  if (!expected) {
