...
// processStockUnavailableRefunds) — même esprit que STALE_THRESHOLD_MS dans
// payment-api/src/payment/payment-reconciliation.service.ts.
 ...
 * après libération de la capacité (PAID_STOCK_UNAVAILABLE, voir
 * reconcileTicketPayment dans src/lib/tickets.ts, seul appelant) et tente
 * immédiatement la demande de remboursement auprès de payment-api. Ne lève
 * jamais : un payment-api indisponible laisse le dossier à
 * PENDING_REFUND_REQUEST, repris par processStockUnavailableRefunds()
 * (scheduler) — reconcileTicketPayment ne doit jamais échouer à cause de ce
 * traitement best-effort.
 */
...
async function tryRequestRefund(dossier: StockUnavailableRefund): Promise<StockUnavailableRefundStatus | null> {
  ...
  try {
    const refund = await requestRefund({
      ...
      reason: "Paiement confirmé après libération de la capacité (stock indisponible).",
 ...
 * MANUAL_REVIEW) en relisant payment-api — jamais fait confiance à l'état
 * local seul, qui peut avoir divergé (ex. un opérateur a confirmé le
 * remboursement manuel côté payment-api entre-temps). Alerte (log
 ...
 * et, en option, par un ordonnanceur externe (voir
