...
/**
 * Tente d'enregistrer `eventId` comme traité (TS-14, avancement.md) — la clé
 * primaire sur `event_id` fait office de verrou : un seul appel concurrent
 * (même en cas de course entre deux retries reçus quasi simultanément)
 * réussit son INSERT et doit traiter l'événement métier ; les autres
 * échouent sur la contrainte d'unicité et savent qu'ils peuvent l'ignorer.
 * Retourne `true` seulement pour le tout premier appel avec cet eventId.
 */
