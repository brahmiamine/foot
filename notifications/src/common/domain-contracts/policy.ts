/**
 * Copie locale de `packages/domain-contracts/src/policy.ts` (GOV-001).
 *
 * `@foot/domain-contracts` est un paquet ESM pur (`"type": "module"`,
 * exports TS-source-only). Les apps Next.js qui l'importent par chemin
 * relatif (club-hub, identity, ticketing...) s'en sortent parce que leur
 * bundler gère l'interop ESM/CJS. `notifications` est un service NestJS
 * CommonJS exécuté nu (`node dist/main.js`, aucun bundler) : un `tsc`
 * "nodenext" compile ce fichier externe en respectant le `"type": "module"`
 * de son propre package.json, ce qui produit un `export function ...` ESM
 * dans `dist/`, que `require()` ne peut pas charger (`SyntaxError:
 * Unexpected token 'export'`) — vérifié en buildant réellement le service.
 * D'où cette copie, gardée strictement identique à la source pour rester
 * facilement diffable. Ne PAS remplacer par un import relatif vers
 * `packages/domain-contracts` tant que ce service reste CommonJS.
 */
...
export interface ResolvedPolicy<T extends object> {
  ...
  appliedRecords: Array<
    Pick<PolicyRecord<T>, 'id' | 'scopeType' | 'scopeId' | 'version'>
  >;
}
...
function asTimestamp(value: string | Date | null | undefined): number | null {
  ...
  if (!Number.isFinite(timestamp))
    throw new Error(`Invalid policy effective date: ${String(value)}`);
  return timestamp;
}

export function isPolicyRecordActive<T extends object>(
...
export function policyScopeMatchesContext<T extends object>(
  ...
  switch (record.scopeType) {
    ...
    case 'FEDERATION':
      return Boolean(
        context.federationId && record.scopeId === context.federationId,
      );
    case 'LEAGUE':
    ...
    case 'COMPETITION':
      return Boolean(
        context.competitionId && record.scopeId === context.competitionId,
      );
    case 'CLUB':
...
function selectLatestActiveSnapshot<T extends object>(
...
export function resolvePolicy<T extends object>(
  ...
  const matching = records.filter(
    (record) =>
      policyScopeMatchesContext(record, context) &&
      isPolicyRecordActive(record, at),
  );

  for (const record of matching) {
    if (!Number.isInteger(record.version) || record.version < 1) {
      throw new Error(
        `Policy ${record.id} has invalid version ${record.version}`,
      );
    }
    for (const key of Object.keys(record.values)) {
      if (!knownKeys.has(key))
        throw new Error(`Policy ${record.id} contains unknown key ${key}`);
    }
  }
  ...
  const selected = Array.from(byScope.values())
    ...
    .sort(
      (a, b) =>
        POLICY_SCOPE_PRECEDENCE[a.scopeType] -
        POLICY_SCOPE_PRECEDENCE[b.scopeType],
    );
