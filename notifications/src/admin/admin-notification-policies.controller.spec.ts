...
function user(role: 'ADMIN' | 'SUPERADMIN', teamId: string | null): AuthenticatedUser {
  return {
...
describe('AdminNotificationPoliciesController effective policy (GOV-006)', () => {
  it('forces ADMIN resolution to the authenticated club', async () => {
    const resolveExplained = jest.fn().mockResolvedValue({ values: {}, sources: {}, appliedRecords: [] });
    const controller = new AdminNotificationPoliciesController({ resolveExplained } as never);
  ...
  it('rejects an ADMIN without a club scope', async () => {
    const controller = new AdminNotificationPoliciesController({ resolveExplained: jest.fn() } as never);

    await expect(
      controller.resolveEffective(user('ADMIN', null), 'MARKETING', null, undefined),
  ...
  it('rejects an invalid resolution date', async () => {
    ...
    await expect(
      controller.resolveEffective(user('SUPERADMIN', null), 'MARKETING', null, 'not-a-date'),
