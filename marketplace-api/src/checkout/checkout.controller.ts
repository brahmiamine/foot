 ...
 * cart/cart.controller.ts) : marketplace-api n'a pas de notion de session
...
export class CheckoutController {
   ...
   * paiement PAID/FAILED/EXPIRED (voir payment-api/src/webhooks). Le corps
   ...
   * de payment-api avant toute décision — même pattern que
   * billetterie/teamManager (app/api/payments/webhook/route.ts).
   */
  ...
  async webhook(
    ...
    // ce qui permet à payment-api de retenter la livraison du webhook
    // (voir payment-api/src/webhooks/webhook-dispatch.service.ts) — un
    // reconcilePaymentStatus qui échoue (payment-api injoignable, base
