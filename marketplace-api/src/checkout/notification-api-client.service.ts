...
/**
 * TASK-P0-004 (todo.md). Client best-effort vers notification-api's
 ...
 * payment-api/src/notifications/notification-client.service.ts : ne lève
 * jamais — une confirmation de commande ne doit jamais échouer parce que
 * notification-api est indisponible ou non configuré.
 */
@Injectable()
  ...
  async notify(payload: NotifyMemberPayload): Promise<void> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        this.logger.error(
          `notification-api rejected type=${payload.type}: ${response.status} ${text}`,
        );
      }
    } catch (error) {
      this.logger.error(
        `Failed to reach notification-api for type=${payload.type}: ${
