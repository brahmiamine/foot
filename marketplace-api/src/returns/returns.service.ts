...
/** Statuts de remboursement chez payment-api qui ne sont pas encore terminaux. */
...
export class ReturnsService {
   ...
   * payment-api (TASK-P0-001) pour le montant de la ligne retournée
   ...
   * (voir payment-api/README.md § Limites connues : un remboursement
   ...
   * payment-api répond déjà SUCCEEDED (Flouci, remboursement total unique).
   */
   ...
   * initiale a échoué (payment-api indisponible, réponse invalide, etc.).
   * Réutilise la même clé d'idempotence (`return:<id>`) : si la demande
   * initiale avait en réalité été acceptée côté payment-api malgré une
  ...
  /**
   * TASK-P0-006 — relit auprès de payment-api le statut d'un remboursement
   ...
   * acheteur (best-effort via notification-api) de l'issue courante du
