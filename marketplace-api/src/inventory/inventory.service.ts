...
export class InventoryService {
   ...
   * nom de `referenceId` (opaque — un identifiant de ligne de commande une
   * fois TASK-P0-004 construit ; jamais interprété ici). Transactionnelle et
  ...
  async releaseReservation(reservationId: string): Promise<StockReservation> {
    return this.resolveReservation(reservationId, ReservationStatus.RELEASED);
  }
   ...
   * TTL est dépassé — appelée par un scheduler périodique une fois
   * TASK-P0-004 construit (aucun appelant aujourd'hui, ce service n'a pas
   * encore de créateur de réservations en dehors des tests). Un échec de
   * résolution individuel (course avec une confirmation/relâche
   * concurrente entre la lecture et l'appel) n'interrompt jamais le lot :
   * une réservation de moins à expirer, pas une erreur à propager.
   */
