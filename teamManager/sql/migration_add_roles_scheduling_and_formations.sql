...
-- entraînements et composition (formation terrain) pour teamManager.
--
...
-- `team_id` -> `teams.id` (table partagée avec ArbiNote/cardManager/matchsheet).
...
--     supporter les matchs amicaux (propres à teamManager) en plus des
...
-- ---------------------------------------------------------------------
-- 2. Rôles & permissions (RBAC applicatif, propre à teamManager).
...
-- ---------------------------------------------------------------------
-- 3. Matchs amicaux — propres à teamManager (les matchs officiels restent
