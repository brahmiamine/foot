 ...
 * ../../marketplace-api/README.md). Remplace l'accès cross-DB direct que
 * teamManager avait auparavant vers `sp_products`/`sp_sellers` (voir
...
async function apiFetch<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`marketplace-api ${response.status} sur ${path}: ${body}`);
  }
