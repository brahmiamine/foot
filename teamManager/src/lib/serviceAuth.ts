 ...
 * interruption via `TEAMMANAGER_SERVICE_API_KEY_PREVIOUS` (acceptée en
 * parallèle jusqu'à `TEAMMANAGER_SERVICE_API_KEY_PREVIOUS_EXPIRES_AT`) et
...
export function ensureServiceAuth(request: NextRequest): NextResponse | null {
  const current = process.env.TEAMMANAGER_SERVICE_API_KEY;
  if (!current) {
  ...
  if (provided === current) {
    ...
    const previous = process.env.TEAMMANAGER_SERVICE_API_KEY_PREVIOUS;
    const previousExpiresAt = process.env.TEAMMANAGER_SERVICE_API_KEY_PREVIOUS_EXPIRES_AT;
