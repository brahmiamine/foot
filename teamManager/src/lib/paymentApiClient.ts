 ...
 * Copie du client de `billetterie` (../billetterie/src/lib/paymentApiClient.ts,
 * premier et jusqu'ici seul vrai appelant de payment-api du dépôt) — même
 * forme de requête, même variables d'environnement, pour rester cohérent
 * entre apps. N'accepte que les providers dont le DTO d'initiation ne
 * demande que orderId/amount/email/userId (Konnect, Flouci) : Paymee exige
 * en plus firstName/lastName/phoneNumber, des champs que le profil MEMBER
 * de `sso` ne collecte pas aujourd'hui — non supporté ici tant que ce
 * formulaire de checkout n'existe pas.
 */
...
function getProviderInitPath(): string {
  ...
  if (!path) {
    throw new Error(
      `PAYMENT_PROVIDER="${provider}" non supporté par teamManager (konnect ou flouci uniquement) — voir src/lib/paymentApiClient.ts.`
    );
  }
  return path;
}
