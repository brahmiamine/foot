...
/**
 * Client serveur-à-serveur vers notification-api pour la gestion de
 * l'abonnement Web Push du compte connecté (staff club — convocations,
 * annonces déjà émises par teamManager via lib/notificationClient.ts) :
 * teamManager ne signe jamais de requête en son nom, il relaie le JWT `sso` de
 * l'utilisateur courant (Authorization: Bearer, voir
 * notification-api/src/auth/guards/jwt-auth.guard.ts). Même convention que
 * ob/src/lib/notificationApi.ts, réduite aux seuls abonnements push.
 */
