 ...
 * crashe entre les deux ou si notification-api est injoignable) par un
 * outbox transactionnel — même principe que `payment_outbox_events`
 * (payment-api, TS-12/TS-13). `NotificationOutboxService.enqueue()` insère
...
export class NotificationOutboxEvent {
  ...
  // voir retry-schedule.ts (même calendrier que payment-api : 1min -> 5min
  // -> 15min -> 1h -> 6h -> FAILED).
