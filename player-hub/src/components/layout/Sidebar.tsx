    ...
    <aside style={{ width: 240, flexShrink: 0, background: "var(--ph-sidebar-bg)", color: "var(--ph-sidebar-text)", minHeight: "100vh", padding: "1.25rem 0.85rem", position: "sticky", top: 0, alignSelf: "flex-start", height: "100vh", overflowY: "auto" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "0 0.4rem 1.5rem" }}>
        {clubLogoUrl ? (
          ...
          <div style={{ width: 34, height: 34, borderRadius: 8, background: "var(--ph-primary)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, color: "#fff", flexShrink: 0 }}>
            PH
          </div>
        )}
        <div>
          <div style={{ fontSize: "0.85rem", fontWeight: 700, color: "#fff" }}>{t("app.name")}</div>
          <div style={{ fontSize: "0.68rem", color: "var(--ph-sidebar-text)" }}>{clubName ?? ""}</div>
        </div>
      </div>
        ...
        {items.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              style={{ display: "flex", alignItems: "center", gap: 10, padding: "0.55rem 0.65rem", borderRadius: 8, fontSize: "0.86rem", fontWeight: active ? 700 : 500, color: active ? "var(--ph-sidebar-text-active)" : "var(--ph-sidebar-text)", background: active ? "var(--ph-sidebar-active-bg)" : "transparent", marginBottom: 2 }}
            >
