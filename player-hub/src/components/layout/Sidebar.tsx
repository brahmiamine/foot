...
export function Sidebar({ clubName, clubLogoUrl }: { clubName?: string; clubLogoUrl?: string | null }) {
  ...
  const { t } = useI18n();
  return <aside style={{ width: 240, flexShrink: 0, background: "var(--ph-sidebar-bg)", color: "var(--ph-sidebar-text)", minHeight: "100vh", padding: "1.25rem 0.85rem", position: "sticky", top: 0, alignSelf: "flex-start", height: "100vh", overflowY: "auto" }}>
    <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "0 0.4rem 1.5rem" }}>
      {clubLogoUrl ? <>{/* eslint-disable-next-line @next/next/no-img-element -- logo distant fourni par le club */}<img src={clubLogoUrl} alt="" width={34} height={34} style={{ borderRadius: 8, objectFit: "cover", flexShrink: 0 }} /></> : <div style={{ width: 34, height: 34, borderRadius: 8, background: "var(--ph-primary)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, color: "#fff", flexShrink: 0 }}>PH</div>}
