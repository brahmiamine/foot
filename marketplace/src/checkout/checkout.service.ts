 ...
 * payments) ; un échec compense explicitement (annule la commande,
...
export class CheckoutService {
   ...
   * re-tentée (elle-même idempotente côté payments par la même clé).
   */
  ...
  private async createOrderWithReservations(
    ...
    // ShopOrderService.createOrder côté club-hub).
   ...
   * CheckoutController/le webhook payments.
   */
  ...
  /**
   * payments ne rappelle jamais directement CheckoutService : c'est à
   * marketplace de relire son statut, soit via le webhook applicatif
   * signé (CheckoutController), soit via le scheduler de réconciliation
   * (CheckoutReconciliationService) — même stratégie que
   * ticketing/src/lib/tickets.ts#reconcileTicketPayment. Idempotente :
