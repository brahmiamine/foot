...
import { PaymentApiClientService } from './payments-client.service';
import { NotificationApiClientService } from './notifications-client.service';
 ...
 * mock of TypeORM. Same approach as payments/refund.service.spec.ts and
 * marketplace/inventory.service.spec.ts.
 */
class FakeDb {
...
describe('CheckoutService', () => {
  ...
  describe('checkout', () => {
    ...
    it('cancels the order and releases reserved stock when payment initiation fails', async () => {
      ...
      paymentApiClient.initPayment.mockRejectedValue(
        new Error('payments unreachable'),
      );
      ...
      ).rejects.toThrow('payments unreachable');
    ...
    it('is idempotent: replaying the same idempotency key does not create a second order', async () => {
      ...
      // initPayment rappelé (payments est lui-même idempotent par la
  ...
  describe('reconcilePaymentStatus', () => {
    it('confirms the order when payments reports PAID (webhook / late poll)', async () => {
      db.sellers.set('seller-1', buildSeller());
      await seedCartItem('member-1', buildProduct(), 1, { available: 10 });
      paymentApiClient.initPayment.mockResolvedValue({
        paymentId: 'pay-1',
    ...
    it('cancels and releases stock when payments reports FAILED (late webhook after a lost delivery)', async () => {
      db.sellers.set('seller-1', buildSeller());
      await seedCartItem('member-1', buildProduct(), 1, { available: 10 });
      paymentApiClient.initPayment.mockResolvedValue({
        paymentId: 'pay-1',
