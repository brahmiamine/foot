...
const SENSITIVE_FIELDS: Array<keyof UpdateProductDto> = [
  'name',
  'description',
  'shortDescription',
  'categoryId',
...
export class ProductsService {
  ...
  findAllBySeller(sellerId: string): Promise<Product[]> {
  ...
  private async policyForSeller(
    sellerId: string,
  ): Promise<{ clubId: string; settings: MarketplaceClubSettings }> {
    ...
    const settings =
      (await repo.findOne({ where: { clubId: seller.clubId } })) ??
      repo.create({
  ...
  private hasSensitiveChange(dto: UpdateProductDto): boolean {
    return SENSITIVE_FIELDS.some((field) => dto[field] !== undefined);
  }
  private notifyReview(
    ...
    return this.centralNotifications.notifyClubAdmins(
      clubId,
      'PRODUCT_SUBMITTED',
      'Produit à approuver',
      `${product.name} nécessite une validation du club (${reason}).`,
  ...
  async update(
    ...
    if (
      [ProductStatus.APPROVED, ProductStatus.PUBLISHED].includes(product.status)
    ) {
      const { clubId, settings } = await this.policyForSeller(sellerId);
      if (
        settings.productApprovalRequired &&
        settings.productReapprovalOnSensitiveChange &&
        this.hasSensitiveChange(dto)
      ) {
    ...
    if (dto.images === undefined) saved = await this.repository.save(product);
      ...
      saved = await this.dataSource.transaction(async (manager) => {
        const productRepository = manager.getRepository(Product);
        const persisted = await productRepository.save(product);
        const imageRepository = manager.getRepository(ProductImage);
    ...
    if (notifyClubId)
      await this.notifyReview(notifyClubId, saved, 'modification sensible');
    return saved;
  }
  ...
  async sellerTransition(
    ...
    if (nextStatus === ProductStatus.SUBMITTED) {
      const { clubId, settings } = await this.policyForSeller(sellerId);
      product.status = settings.productApprovalRequired
      ...
      if (settings.productApprovalRequired)
        await this.notifyReview(clubId, saved, 'nouvelle soumission');
      return saved;
    }
