...
async function bootstrap() {
  const app = await NestFactory.create(AppModule, {
    ...
    logger: new StructuredLoggerService('marketplace', {
      logLevels: ['error', 'warn', 'log'],
    }),
    // TASK-P0-004 : expose request.rawBody (Buffer), nécessaire pour
    // vérifier la signature HMAC du webhook payments sur les octets
  ...
  Logger.log(`marketplace listening on port ${port}`, 'Bootstrap');
}
