...
describe('ReturnsService', () => {
  ...
  describe('complete (US-52) — TASK-P0-006 : déclenche le remboursement', () => {
    ...
    it('a failed refund request (payments unreachable) moves to REFUND_FAILED, visible with the error', async () => {
      mockCompleteContext();
      ...
      expect(result.refundError).toBe('payments unreachable');
      expect(notificationsService.notify).toHaveBeenCalledWith(
        'seller-1',
    ...
    it('re-attempts the refund with the same idempotency key and clears the previous error on success', async () => {
      repository.findOne.mockResolvedValue(
        buildReturn({
          ...
          refundError: 'payments unreachable',
        }),
      );
      sellerOrderRepository.findOne.mockResolvedValue(
  ...
  describe('reconcileRefund (TASK-P0-006) — filet de sécurité périodique', () => {
    it('does nothing if payments still reports the same non-terminal status', async () => {
    ...
    it('resolves to REFUNDED once payments reports SUCCEEDED (financial confirmation)', async () => {
      const returnRequest = buildReturn({
        status: ReturnStatus.COMPLETED,
        refundId: 'refund-1',
        refundStatus: 'MANUAL_REVIEW',
    ...
    it('resolves to REFUND_FAILED once payments reports FAILED', async () => {
      const returnRequest = buildReturn({
        status: ReturnStatus.COMPLETED,
        refundId: 'refund-1',
        refundStatus: 'MANUAL_REVIEW',
