...
export class AuthController {
  ...
  @Post('login') async login(@Body() dto: LoginDto): Promise<{ token: string; sellerId: string; role: string }> {
    const user = await this.sellersService.findUserByEmail(dto.email);
    if (!user || !(await this.sellersService.verifyPassword(user, dto.password))) throw new UnauthorizedException('Identifiants invalides');
    if (user.status !== SellerUserStatus.ACTIVE || user.seller.status !== SellerStatus.ACTIVE) throw new UnauthorizedException('Compte vendeur inactif');
    const token = await this.sellerJwtService.sign({ sellerUserId: user.id, sellerId: user.sellerId, role: user.role });
