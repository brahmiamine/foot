...
export class CentralNotificationsClient {
  ...
  private async dispatch(payload: InternalNotificationPayload): Promise<void> {
    ...
    if (!baseUrl || !apiKey) {
      this.logger.warn(`Notification ${payload.type} skipped: notifications service is not configured`);
      return;
    }
    try {
      ...
      if (!response.ok) this.logger.warn(`Notification ${payload.type} failed with HTTP ${response.status}`);
    } catch (error) {
      this.logger.warn(`Notification ${payload.type} failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }

  notifyClubAdmins(clubId: string, type: string, title: string, body: string, data?: Record<string, unknown>, eventId?: string): Promise<void> {
