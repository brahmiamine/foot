...
describe('VariantsService', () => {
  let variantRepository: { findOne: jest.Mock; save: jest.Mock; delete: jest.Mock };
  ...
  beforeEach(() => {
    ...
    productRepository = { findOne: jest.fn().mockResolvedValue({ id: 'product-1', sellerId: 'seller-1' } as Product) };
    txVariantRepository = {
    ...
    const manager = {
      getRepository: jest.fn((entity) => entity === ProductVariant ? txVariantRepository : entity === InventoryItem ? txInventoryRepository : undefined),
    };
  ...
  it('creates the variant and its initial stock atomically', async () => {
    await service.create('product-1', 'seller-1', {
      sku: 'MAILLOT-M', attributes: { Taille: 'M' }, imageUrl: 'https://cdn/x.jpg', initialStock: 4,
    });
    expect(dataSource.transaction).toHaveBeenCalledTimes(1);
    expect(txInventoryRepository.save).toHaveBeenCalledWith(
      expect.objectContaining({ sellerId: 'seller-1', productId: 'product-1', variantId: 'variant-1', available: 4 }),
    );
  });

  it('rejects when the product does not belong to the seller', async () => {
    productRepository.findOne.mockResolvedValue(null);
    await expect(service.toggleActive('variant-1', 'product-1', 'seller-1')).rejects.toThrow(NotFoundException);
  });

  it('updates image and active state through Marketplace ownership', async () => {
    variantRepository.findOne.mockResolvedValue({
      id: 'variant-1', productId: 'product-1', sku: 'MAILLOT-M', attributes: { Taille: 'M' }, price: null,
      imageUrl: null, isActive: true,
    } as ProductVariant);
    const result = await service.update('variant-1', 'product-1', 'seller-1', {
      imageUrl: 'https://cdn/new.jpg', isActive: false,
    });
