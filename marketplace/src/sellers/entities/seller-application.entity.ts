...
export interface SellerApplicationDocument { label: string; url: string }
...
export class SellerApplication {
  ...
  @Column({ type: 'json', nullable: true }) documents: SellerApplicationDocument[] | null;
  @Column({ type: 'enum', enum: SellerApplicationStatus, default: SellerApplicationStatus.PENDING }) status: SellerApplicationStatus;
  @Column({ type: 'text', nullable: true }) rejectionReason: string | null;
  @Column({ type: 'varchar', length: 191, nullable: true }) reviewedBy: string | null;
