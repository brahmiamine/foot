...
export class SellersService {
  constructor(
    ...
    @InjectRepository(SellerInvite)
    private readonly inviteRepository: Repository<SellerInvite>,
    @InjectRepository(MarketplaceClubSettings)
    private readonly settingsRepository: Repository<MarketplaceClubSettings>,
  ...
  async updateClubSettings(
    clubId: string,
    dto: UpdateMarketplaceSettingsDto,
  ): Promise<MarketplaceClubSettings> {
    ...
    if (dto.sellerApplicationsEnabled !== undefined)
      settings.sellerApplicationsEnabled = dto.sellerApplicationsEnabled;
    if (dto.sellerApprovalRequired !== undefined)
      settings.sellerApprovalRequired = dto.sellerApprovalRequired;
    if (dto.productApprovalRequired !== undefined)
      settings.productApprovalRequired = dto.productApprovalRequired;
    if (dto.productReapprovalOnSensitiveChange !== undefined)
      settings.productReapprovalOnSensitiveChange =
        dto.productReapprovalOnSensitiveChange;
  ...
  async createApplication(
    ...
    if (!settings.sellerApplicationsEnabled)
      throw new ConflictException(
        'Les demandes vendeurs sont actuellement fermées pour ce club',
      );
    const email = dto.email.trim().toLowerCase();
    if (await this.sellerRepository.findOne({ where: { email } }))
      throw new ConflictException(
        'Un compte vendeur existe déjà avec cet email',
      );
    const existing = await this.applicationRepository.findOne({
      where: [
        { clubId: dto.clubId, email, status: SellerApplicationStatus.PENDING },
        {
          clubId: dto.clubId,
          email,
          status: SellerApplicationStatus.UNDER_REVIEW,
        },
      ],
    });
    if (existing)
    ...
    if (!settings.sellerApprovalRequired)
      return (
    ...
    await this.centralNotifications.notifyClubAdmins(
      application.clubId,
      'SELLER_APPLICATION_CREATED',
      'Nouvelle demande vendeur',
      `${application.businessName} a envoyé une demande pour vendre sur la boutique du club.`,
      { applicationId: application.id, businessName: application.businessName },
  ...
  async findApplication(
    ...
    if (!application)
      throw new NotFoundException('Demande vendeur introuvable pour ce club');
    return application;
  }
  async markApplicationUnderReview(
    ...
    if (application.status !== SellerApplicationStatus.PENDING)
      throw new ConflictException(
        `Demande non révisable dans le statut ${application.status}`,
      );
  ...
  async approveApplication(
    ...
    if (
      ...
      throw new ConflictException(
        `Demande non approuvable dans le statut ${application.status}`,
      );
    ...
    const result = await this.dataSource.transaction(async (manager) => {
      if (
        (await manager.findOne(Seller, {
          where: { email: application.email },
        })) ||
        (await manager.findOne(SellerUser, {
          where: { email: application.email },
        }))
      )
        throw new ConflictException('Cet email est déjà utilisé');
      const seller = await manager.save(
        manager.create(Seller, {
          ...
          activityCategory: application.activityCategory,
          taxId: application.taxId,
          tradeRegister: application.tradeRegister,
      ...
      const user = await manager.save(
        manager.create(SellerUser, {
          sellerId: seller.id,
          name: application.ownerName,
          email: application.email,
          passwordHash: temporaryPasswordHash,
          role: SellerUserRole.OWNER,
          status: SellerUserStatus.INVITED,
          emailVerifiedAt: null,
        }),
      );
      await manager.save(
        manager.create(SellerInvite, {
          sellerId: seller.id,
    ...
    const portalUrl =
      this.config.get<string>('SELLER_PORTAL_URL') ?? 'http://localhost:3006';
    const activationUrl = `${portalUrl.replace(/\/$/, '')}/activate?token=${encodeURIComponent(rawToken)}`;
    await this.centralNotifications.notifyExternalEmail(
      application.email,
      application.ownerName,
      application.clubId,
      'SELLER_APPLICATION_APPROVED',
      'Votre demande vendeur a été approuvée',
      `Votre demande pour ${application.businessName} a été approuvée. Activez votre compte vendeur ici : ${activationUrl}`,
      {
      ...
      `seller-application:${application.id}:approved`,
  ...
  async rejectApplication(
    ...
    if (
      ...
      throw new ConflictException(
        `Demande non rejetable dans le statut ${application.status}`,
      );
    ...
    await this.centralNotifications.notifyExternalEmail(
      ...
      `Votre demande pour ${saved.businessName} a été refusée. Motif : ${saved.rejectionReason}`,
      { applicationId: saved.id, rejectionReason: saved.rejectionReason },
      `seller-application:${saved.id}:rejected`,
    );
    return saved;
  }
  ...
  async activateInvitation(
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      const user = await manager
        .getRepository(SellerUser)
        .findOne({ where: { id: invite.sellerUserId } });
      if (!user || user.status !== SellerUserStatus.INVITED)
        throw new GoneException("Ce lien d'activation n'est plus utilisable");
  ...
  findUserByEmail(email: string): Promise<SellerUser | null> {
    return this.sellerUserRepository.findOne({
      where: { email: email.trim().toLowerCase() },
      relations: { seller: true },
    });
  }
  verifyPassword(user: SellerUser, password: string): Promise<boolean> {
  ...
  findAllByClub(clubId: string, status?: SellerStatus): Promise<Seller[]> {
  ...
  async updateCommission(
    ...
    seller.commissionRate = commissionRate.toFixed(2);
    return this.sellerRepository.save(seller);
  }
  async updateStatus(
    sellerId: string,
    ...
    if (saved.status === SellerStatus.SUSPENDED) {
      await this.sellerNotifications.notify(
        saved.id,
        NotificationType.SELLER_SUSPENDED,
        'Compte suspendu',
        dto.reason ?? 'Votre compte vendeur a été suspendu par le club.',
      );
      await this.centralNotifications.notifyExternalEmail(
        saved.email,
        saved.ownerName,
