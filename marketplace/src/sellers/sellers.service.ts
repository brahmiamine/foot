...
export class SellersService {
  ...
  async createApplication(dto: CreateSellerApplicationDto): Promise<SellerApplication> {
    ...
    const application = await this.applicationRepository.save(this.applicationRepository.create({
      ...dto, email, phone: dto.phone ?? null, address: dto.address ?? null, city: dto.city ?? null, country: dto.country ?? null,
      description: dto.description ?? null, activityCategory: dto.activityCategory ?? null, taxId: dto.taxId ?? null,
      tradeRegister: dto.tradeRegister ?? null, documents: dto.documents ?? null, status: SellerApplicationStatus.PENDING,
      rejectionReason: null, reviewedBy: null, reviewedAt: null, sellerId: null,
    }));
  ...
  findApplicationsByClub(clubId: string, status?: SellerApplicationStatus): Promise<SellerApplication[]> {
    return this.applicationRepository.find({ where: status ? { clubId, status } : { clubId }, order: { createdAt: 'DESC' } });
  }

  async findApplication(id: string, clubId: string): Promise<SellerApplication> {
    const application = await this.applicationRepository.findOne({ where: { id, clubId } });
    if (!application) throw new NotFoundException('Demande vendeur introuvable pour ce club');
    return application;
  }

  async markApplicationUnderReview(id: string, clubId: string, actorId: string): Promise<SellerApplication> {
    const application = await this.findApplication(id, clubId);
    if (application.status !== SellerApplicationStatus.PENDING) throw new ConflictException(`Demande non révisable dans le statut ${application.status}`);
    application.status = SellerApplicationStatus.UNDER_REVIEW;
  ...
  async approveApplication(id: string, clubId: string, actorId: string): Promise<{ application: SellerApplication; seller: Seller }> {
    ...
    const temporaryPasswordHash = await bcrypt.hash(randomBytes(48).toString('hex'), SALT_ROUNDS);
    ...
    const result = await this.dataSource.transaction(async (manager) => {
      ...
      application.status = SellerApplicationStatus.APPROVED; application.reviewedBy = actorId; application.reviewedAt = new Date(); application.rejectionReason = null; application.sellerId = seller.id;
      await manager.save(application);
      return { seller };
    });
    const portalUrl = this.config.get<string>('SELLER_PORTAL_URL') ?? 'http://localhost:3006';
    const activationUrl = `${portalUrl.replace(/\/$/, '')}/activate?token=${encodeURIComponent(rawToken)}`;
  ...
  async rejectApplication(id: string, clubId: string, actorId: string, reason?: string): Promise<SellerApplication> {
    if (!reason?.trim()) throw new ConflictException('Un motif de rejet est obligatoire');
    const application = await this.findApplication(id, clubId);
    if (![SellerApplicationStatus.PENDING, SellerApplicationStatus.UNDER_REVIEW].includes(application.status)) throw new ConflictException(`Demande non rejetable dans le statut ${application.status}`);
    application.status = SellerApplicationStatus.REJECTED; application.rejectionReason = reason.trim(); application.reviewedBy = actorId; application.reviewedAt = new Date();
    const saved = await this.applicationRepository.save(application);
    await this.centralNotifications.notifyExternalEmail(saved.email, saved.ownerName, saved.clubId, 'SELLER_APPLICATION_REJECTED', 'Votre demande vendeur a été refusée', `Votre demande pour ${saved.businessName} a été refusée. Motif : ${saved.rejectionReason}`, { applicationId: saved.id, rejectionReason: saved.rejectionReason }, `seller-application:${saved.id}:rejected`);
    return saved;
  }

  async activateInvitation(token: string, password: string): Promise<SellerUser> {
    const invite = await this.inviteRepository.findOne({ where: { tokenHash: this.hashToken(token) } });
    ...
    const user = await this.sellerUserRepository.findOne({ where: { id: invite.sellerUserId } });
    if (!user || user.status !== SellerUserStatus.INVITED) throw new GoneException("Ce lien d'activation n'est plus utilisable");
    user.passwordHash = await bcrypt.hash(password, SALT_ROUNDS); user.status = SellerUserStatus.ACTIVE; user.emailVerifiedAt = new Date(); invite.usedAt = new Date();
    await this.dataSource.transaction(async (manager) => { await manager.save(user); await manager.save(invite); });
    return user;
  }
  ...
  async updateStatus(sellerId: string, clubId: string, dto: UpdateSellerStatusDto): Promise<Seller> {
    const seller = await this.sellerRepository.findOne({ where: { id: sellerId, clubId } });
    if (!seller) throw new NotFoundException('Vendeur introuvable pour ce club');
    const allowed = CLUB_ALLOWED_SELLER_TRANSITIONS[seller.status] ?? [];
    if (!allowed.includes(dto.status)) throw new ConflictException(`Transition ${seller.status} → ${dto.status} non autorisée`);
    seller.status = dto.status; seller.statusReason = dto.reason ?? null;
    const saved = await this.sellerRepository.save(seller);
    if (saved.status === SellerStatus.SUSPENDED) {
      await this.sellerNotifications.notify(saved.id, NotificationType.SELLER_SUSPENDED, 'Compte suspendu', dto.reason ?? 'Votre compte vendeur a été suspendu par le club.');
      await this.centralNotifications.notifyExternalEmail(saved.email, saved.ownerName, saved.clubId, 'SELLER_SUSPENDED', 'Compte vendeur suspendu', dto.reason ?? 'Votre compte vendeur a été suspendu par le club.', { sellerId: saved.id });
    } else if (saved.status === SellerStatus.ACTIVE) {
      await this.sellerNotifications.notify(saved.id, NotificationType.SELLER_ACTIVATED, 'Compte actif', 'Votre compte vendeur est actif.');
    }
    return saved;
  }
}
