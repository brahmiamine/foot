...
**État global (13 août 2026) :** les 5 phases ont un premier passage backend complet (modèle de données, transactions, guards serveur, tests). Un premier lot d'écrans `superadmin` a été ajouté pour consommer ces routes (provisioning, affiliations, officiels de match, création de transfert — voir détail par phase) ; chaque phase reste marquée `[~]` car il manque encore, selon les cas, l'ouverture aux rôles `FEDERATION_ADMIN`/`LEAGUE_ADMIN` (les nouveaux écrans restent réservés à `PLATFORM_SUPERADMIN`, comme le reste de `superadmin`), un tableau de bord Transferts complet, la vue globale `/joueurs`, ou toute UI côté `arbinote` (Phase 5, encore 100% API-only). Le détail de ce qui reste par phase est dans chaque section ci-dessous ("Reste ouvert").
...
- [ ] **Reste ouvert** : écrans `arbinote`/`superadmin` (aucune UI, API-only) ; édition d'un rapport `DRAFT` déjà créé (seule la création initiale est couverte, pas de `PATCH`) ; agrégats plus riches (tendance dans le temps, comparaison entre observateurs) au-delà de la moyenne simple.
