...
**État global (13 août 2026) :** les 5 phases ont un premier passage backend complet (modèle de données, transactions, guards serveur, tests), et chacune a maintenant au moins un écran qui consomme ses routes. Deuxième passage : les écrans `superadmin` de provisioning/affiliations/officiels de match/transferts sont désormais aussi accessibles à `FEDERATION_ADMIN` (pas seulement `PLATFORM_SUPERADMIN`) — soit directement, soit via trois nouveaux écrans dédiés (`/admin/clubs-affilies`, `/admin/officiels-matchs`) qui évitent d'ouvrir les gros écrans génériques `/admin/teams`/`/admin/matches` (ceux-ci restent `PLATFORM_SUPERADMIN`-only, voir Phase 1). Chaque phase reste marquée `[~]` car il manque encore, selon les cas, un tableau de bord Transferts complet (historique/prêts/annulés), la vue globale `/joueurs`, ou (Phase 1) le scoping des ~40 routes hors migration.md des gros écrans génériques. Le détail de ce qui reste par phase est dans chaque section ci-dessous ("Reste ouvert").
...
- [ ] **Reste ouvert (reporté, hors périmètre "modèle d'autorisation")** : les gros écrans génériques `/admin/teams` (CRUD complet équipe : création, import, branding, suppression) et `/admin/matches` (CRUD complet match) restent `PLATFORM_SUPERADMIN`-only — les ouvrir à `FEDERATION_ADMIN` demanderait de scoper leurs routes d'écriture par fédération (aujourd'hui `ensureAdminAuth` simple, sans vérification de propriété), hors périmètre du modèle d'autorisation Fédération/Ligue/Club (ces routes ne gèrent pas des ressources fédérales/ligue mais le référentiel plateforme équipe/match lui-même). Saisons, journées : idem, non scopées. Provisioning des comptes `FEDERATION_ADMIN`/`LEAGUE_ADMIN` : fait, voir "Provisioning des comptes" §0.
