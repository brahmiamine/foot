...
**État global (13 août 2026) :** les 5 phases ont un premier passage backend complet (modèle de données, transactions, guards serveur, tests), et chacune a maintenant au moins un écran qui consomme ses routes (provisioning/affiliations/officiels de match/transferts côté `superadmin`, évaluations officielles côté `arbinote` — voir détail par phase). Chaque phase reste marquée `[~]` car il manque encore, selon les cas, l'ouverture aux rôles `FEDERATION_ADMIN`/`LEAGUE_ADMIN` pour les écrans `superadmin` (restent réservés à `PLATFORM_SUPERADMIN`, contrairement aux écrans `arbinote` qui, eux, sont bien ouverts à `FEDERATION_ADMIN`/`REFEREE_OBSERVER` — voir Phase 5), un tableau de bord Transferts complet, ou la vue globale `/joueurs`. Le détail de ce qui reste par phase est dans chaque section ci-dessous ("Reste ouvert").
...
- **Reste ouvert** : l'écran est réservé à `PLATFORM_SUPERADMIN` (gate de page `hasAdminSession`, inchangé) — un `FEDERATION_ADMIN` autorisé par l'API à inviter `REFEREE`/`MATCH_OFFICIAL`/`REFEREE_OBSERVER` ne peut pas encore se connecter à `superadmin` pour le faire (voir Phase 1 "reste ouvert" : les pages `superadmin` ne sont pas encore ouvertes à `FEDERATION_ADMIN`/`LEAGUE_ADMIN`) ; `PLATFORM_SUPERADMIN` reste hors de ce flux par choix (à provisionner autrement, ex. script `sso/scripts/seed-superadmin.ts` déjà existant).
...
- [ ] **Reste ouvert (reporté, hors périmètre "modèle d'autorisation")** : les routes `superadmin` autres que fédérations/ligues (saisons, journées, clubs affiliés, arbitres, matchs) ne sont pas encore scopées par fédération/ligue — elles restent `PLATFORM_SUPERADMIN`-only comme avant, à traiter au fil des Phases 2-4 quand ces domaines seront eux-mêmes modélisés. **Adaptation des écrans `superadmin` : partiellement faite** — des écrans consommant les routes Phase 2/3/4 existent désormais (voir ces sections), mais toutes les pages `/admin/*` restent gardées par `hasAdminSession()` (`PLATFORM_SUPERADMIN` uniquement, via `ensureAdminAuth`/`GET /api/admin/federations`+`/leagues` eux-mêmes `ensureAdminAuth`-only) : un `FEDERATION_ADMIN`/`LEAGUE_ADMIN` réel ne peut pas encore se connecter à `superadmin` pour utiliser ces écrans lui-même. Ouvrir le gate de page (un helper `getAdminSession`-like existe déjà côté API, pas encore branché sur les pages) et scoper les ~40 routes de listing (fédérations/ligues/saisons/équipes/arbitres) par le `federationId`/`leagueId` de la session reste à faire — actuellement seul `PLATFORM_SUPERADMIN` peut se servir des nouveaux écrans. Provisioning des comptes `FEDERATION_ADMIN`/`LEAGUE_ADMIN` : fait, voir "Provisioning des comptes" §0 (écran ajouté, même limitation).
...
- **Écran (13 août 2026, branche `claude/migration-ui-screens-hok3qk`)** : `/admin/player-transfers` (`AdminPlayerTransfersManager.tsx`) — formulaire de création/homologation (club source → sélecteur joueur peuplé via la nouvelle route `GET /api/admin/teams/:id/players`/`listPlayersForTeam()`, club destination, type, date d'effet, saison, indemnité, dates de prêt) qui appelle `POST /api/admin/player-transfers` (route déjà existante, create+complete v1). Pas encore de tableau de bord : voir "reste ouvert".
- [ ] **Reste ouvert** : vue globale `/joueurs` (§18, filtres fédération/ligue/saison/club/catégorie/poste/statut) et écran `/clubs/{clubId}/joueurs` dans `superadmin` — toujours pas d'UI (nécessiterait une route de listing paginée/filtrée, absente). Écran `Transferts` complet (§23, onglets nouveau/en attente/approuvés/**historique**/prêts/annulés) — seul l'onglet "nouveau" existe (le formulaire ci-dessus) : aucune route `GET` de listing des transferts n'existe côté `superadmin` ni `teamManager` pour peupler les autres onglets, à ajouter avant de construire le reste de l'écran. Workflow de validation à deux acteurs (club source propose, club destination confirme, fédération homologue) — la v1 simplifiée saute cette étape. Notifications (`PLAYER_TRANSFER_*` vers `notification-api`) — non câblées. Vérification temporelle dans `matchsheet` (§22, `player.teamId === match.teamId` actuel à remplacer par une lecture de l'historique d'appartenance à la date du match) — prévue en Phase 4 avec les officiels de match, réutilisera `getAffiliationAt`/l'équivalent côté `cms_team_members`. Historique clubs en lecture seule pour un ancien joueur dans `teamManager` (§23) — non fait. Écran réservé à `PLATFORM_SUPERADMIN` (voir Phase 1 "reste ouvert").
...
- [ ] **Reste ouvert** : vérification temporelle `player.teamId === match.teamId` dans `matchsheet` (§22, distincte des officiels — reste à câbler sur `cms_team_members`/`getAffiliationAt` équivalent côté joueurs) ; audit nominatif enrichi (terminal/IP) au-delà de `assignedBy`/`revokedBy`. Écran réservé à `PLATFORM_SUPERADMIN` (voir Phase 1 "reste ouvert").
...
- [ ] **Reste ouvert** : édition d'un rapport `DRAFT` déjà créé (seule la création initiale est couverte côté service, pas de `PATCH` — l'écran ne permet donc pas de corriger un brouillon, seulement de le soumettre tel quel ou d'en recréer un) ; agrégats plus riches (tendance dans le temps, comparaison entre observateurs) au-delà de la moyenne simple ; pagination de `GET /api/matches`/`GET /api/arbitres` pour les sélecteurs (limite actuelle 200 matchs, pas de recherche câblée dans l'écran alors que les routes la supportent).
