 ...
 *
 * Écrite par superadmin (fédération/ligue, affectation) via les routes
 * internes /api/internal/matches/:matchId/officials (service-à-service,
 * voir lib/serviceAuth.ts) — jamais directement par matchsheet lui-même,
 ...
 * (db/OWNERSHIP.md) : une table dans le schéma de matchsheet, écrite par
...
export class MatchOfficialAssignment {
  ...
  /** Identité de l'auteur de l'affectation (ex: email FEDERATION_ADMIN/LEAGUE_ADMIN côté superadmin) — pas de FK vers `User` : l'auteur n'est pas nécessairement un compte matchsheet. */
