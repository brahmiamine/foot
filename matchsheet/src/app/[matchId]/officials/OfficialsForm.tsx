...
const ROLE_KEYS = { REFEREE_CENTRAL: "refereeCentral", ASSISTANT_1: "assistant1", ASSISTANT_2: "assistant2", FOURTH_OFFICIAL: "fourthOfficial", DELEGATE: "delegate" } as const;
  ...
  return (
    <div className="container-fluid px-0">
      <div className="d-flex justify-content-between align-items-start flex-wrap gap-3 mb-4">
        <div>
          <h1 className="h4 mb-1">{t("officials")}</h1>
...
function OfficialCard({
  ...
  const handleConfirm = async () => {
    if (!fullName.trim()) {
      setError(t("officialNameRequired"));
      return;
    }
    setConfirming(true);
    setError(null);
    ...
    <div className="col-md-4">
      <div className={`card h-100 ${isConfirmed ? "border-success" : ""}`}>
        <div className="card-header bg-transparent d-flex align-items-center justify-content-between">
          ...
          {MANDATORY[role] && <span className="badge bg-danger-subtle text-danger">{t("required")}</span>}
        </div>
        <div className="card-body d-flex flex-column">
          ...
          <div className="mb-3">
            <label className="form-label">{t("license")}</label>
            <input
              type="text"
              className="form-control"
              value={licenseNumber}
          ...
          <div className="d-flex gap-2 mt-auto">
            <button type="button" className="btn btn-outline-secondary btn-sm" onClick={handleSave} disabled={saving || confirming}>
              {saving ? <span className="spinner-border spinner-border-sm" role="status" aria-hidden="true" /> : t("save")}
            </button>
