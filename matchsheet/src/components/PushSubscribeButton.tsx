...
import { registerPushSubscriptionAction, removePushSubscriptionAction } from "@/app/parametres/actions";

const DEVICE_ID_KEY = "matchsheet_push_device_id";
...
export function PushSubscribeButton({ initialSubscriptions }: { initialSubscriptions: PushSubscriptionRecord[] }) {
  ...
  if (!client.supported) {
    return <p className="text-muted small mb-0">Ce navigateur ne prend pas en charge les notifications push.</p>;
  }

  return (
    <div>
      {isThisDeviceSubscribed ? (
        <button className="btn btn-outline-secondary" onClick={unsubscribe} disabled={isPending}>
          Désactiver sur cet appareil
        </button>
      ) : (
        <button className="btn btn-primary" onClick={subscribe} disabled={isPending}>
          Activer les notifications sur cet appareil
        </button>
      )}
      {status && <p className="text-muted small mt-2 mb-0">{status}</p>}
