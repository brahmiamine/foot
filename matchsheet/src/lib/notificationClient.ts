 ...
 * matchsheet lui-même — le JWT `sso` de sa session est relayé en
 * `Authorization: Bearer`, jamais la clé de service (voir
 * notification-api/src/auth/guards/jwt-auth.guard.ts). Lit le cookie
 * directement ici (pas via src/lib/ssoSession.ts, qui reste volontairement
 * Edge-safe pour rester importable par src/middleware.ts) : ce fichier n'est
 * jamais chargé par le middleware, `next/headers` y est donc sans risque.
 */
