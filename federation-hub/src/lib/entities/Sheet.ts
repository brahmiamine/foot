 ...
 * `match-operations` (voir db/OWNERSHIP.md). Seules les colonnes nécessaires à la
 * réouverture pilotée par `federation-hub` sont déclarées ici (pas de relation
 * vers `Match`, pas de colonnes de signature) : SUPERADMIN n'a jamais accès
 * à `match-operations` (choix produit assumé, voir match-operations/src/middleware.ts),
