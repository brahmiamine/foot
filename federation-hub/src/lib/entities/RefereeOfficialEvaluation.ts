...
export class RefereeOfficialEvaluation {
  ...
  /** Rôle SSO de l'évaluateur au moment de la création (audit immuable). */
  @Column({ type: 'varchar', length: 64 })
  evaluator_role!: string

  /** Scope dérivé du match côté serveur, jamais accepté depuis le client. */
  @Column({ type: 'uuid' })
  ...
  /** Critères techniques privés issus de `official_referee_criteria`, séparés de `votes.criteres` et du barème public. */
  @Column({ type: 'json' })
  ...
  /** Identité du FEDERATION_ADMIN ayant validé/rejeté — pas de FK vers `User` (compte staff federation-hub, pas un compte arbinote). */
