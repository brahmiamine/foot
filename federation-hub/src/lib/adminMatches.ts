...
import { reopenSheet } from './matchOperationsClient'
...
export async function updateMatchAdmin(
  ...
  // convocations/effectifs déjà engagés côté match-operations — aucune procédure de
  // correction outillée ici (voir TASK-P0-009), donc on bloque sans dérogation
  // possible plutôt que de laisser un état incohérent entre federation-hub et
  // match-operations (§TASK-P0-008).
  if (teamsChanged) {
...
/**
 * Notifie les deux clubs (staff ADMIN/OBSERVATEUR via notifications)
 ...
 * `match-operations` ne mirrorera plus jamais IN_PROGRESS/FINISHED par-dessus un
 * match CANCELLED (voir SheetService.mirrorMatchStatus) ; `match-operations` bloque
 ...
 * — la saga qui suit (ticketing : fermeture de vente + remboursement des
 * billets payés ; club-hub : annulation des convocations, voir
 * matchSaga.ts) ne peut jamais empêcher ni retarder cette écriture, elle ne
 * fait que compenser ensuite. Un échec de la saga (ticketing/club-hub
 ...
 *
 * TS-31 (avancement.md) : SUPERADMIN n'a pas accès à `match-operations` en tant
 ...
 * désormais pilotée par match-operations lui-même via un appel HTTP authentifié
 * (voir matchOperationsClient.ts, POST /api/internal/matches/:id/reopen) —
 * remplace l'ancien accès direct TypeORM à `ms_sheets`, table propriété de
 * `match-operations`. La lecture de `sheet.status` ci-dessous reste locale
 ...
 * l'écriture change de propriétaire, match-operations revalide de toute façon
