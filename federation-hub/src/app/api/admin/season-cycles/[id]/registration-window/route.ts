...
import { SeasonRegulatoryCycleAuthorizationError, setRegistrationWindow } from '@/lib/seasonRegulatoryCycles'
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json(await setRegistrationWindow(await getDataSource(), session, { userId: session.id, role: session.role, ipAddress: request.headers.get('x-forwarded-for')?.split(',')[0]?.trim() ?? null, userAgent: request.headers.get('user-agent') }, id, body))
  } catch (error) {
    ...
    console.error('Error setting registration window:', error)
