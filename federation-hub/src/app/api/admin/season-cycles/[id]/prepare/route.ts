...
import { prepareSeasonRegulatoryCycle, SeasonRegulatoryCycleAuthorizationError } from '@/lib/seasonRegulatoryCycles'
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json(await prepareSeasonRegulatoryCycle(await getDataSource(), session, {
      userId: session.id,
      role: session.role,
    ...
    console.error('Error preparing season regulatory cycle:', error)
