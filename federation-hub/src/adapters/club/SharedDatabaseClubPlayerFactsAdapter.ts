...
/** Transitional adapter until federation-hub consumes Club-owned player facts over HTTP. */
export class SharedDatabaseClubPlayerFactsAdapter implements ClubPlayerRegulatoryFactsPort {
  async getPlayerRegulatoryFacts(
    ...
    const [membershipRows, suspensionRows, playerRows] = await Promise.all([
      ...
      dataSource.query(`SELECT birthDate FROM Player WHERE id = ? LIMIT 1`, [input.playerId]) as Promise<
        Array<{ birthDate: Date | string | null }>
      >,
    ])
