...
export function useAdminAlerts() {
  ...
  const fetchAlerts = async () => {
    try {
      ...
      const response = await fetch(`/api/admin/arbitrage/arbinote/alerts?${params.toString()}`);
  ...
  const handleResolve = async (alertId: string, notes?: string) => {
    try {
      ...
      const response = await fetch(`/api/admin/arbitrage/arbinote/alerts/${alertId}/resolve`, {
        method: "POST",
        headers: {
  ...
  const handleDismiss = async (alertId: string, notes?: string) => {
    try {
      setResolving(alertId);
      const response = await fetch(`/api/admin/arbitrage/arbinote/alerts/${alertId}/dismiss`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
