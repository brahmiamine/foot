...
export function useCriteres() {
  ...
  async function loadCriteres() {
    try {
      setLoading(true)
      const response = await fetch('/api/admin/arbitrage/arbinote/criteres', {
        cache: 'no-store',
        credentials: 'include',
      })
      if (!response.ok) {
  ...
  async function handleUpdate(event: FormEvent) {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/arbitrage/arbinote/criteres/${editingId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify(payload),
  ...
  async function handleDelete(id: string) {
    ...
    setDeletingId(id)
    try {
      const response = await fetch(`/api/admin/arbitrage/arbinote/criteres/${id}`, {
        method: 'DELETE',
        credentials: 'include',
      })
      if (!response.ok) {
