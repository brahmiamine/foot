...
/** Historique de modération d'une alerte (US-40) — voir /api/admin/arbitrage/arbinote/alerts/[id]/history. */
export function useAlertHistory(alertId: string) {
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      try {
        setLoading(true);
        const response = await fetch(`/api/admin/arbitrage/arbinote/alerts/${alertId}/history`);
