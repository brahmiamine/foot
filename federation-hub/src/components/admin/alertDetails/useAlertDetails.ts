...
export function useAlertDetails(alertId: string) {
  ...
  useEffect(() => {
    const fetchAlert = async () => {
      try {
        setLoading(true);
        // Récupérer l'alerte
        const alertResponse = await fetch(`/api/admin/arbitrage/arbinote/alerts/${alertId}`);
        if (!alertResponse.ok) {
        ...
        const detailsResponse = await fetch(
          `/api/admin/arbitrage/arbinote/votes/${alertData.match_id}/details`
        );
  ...
  const handleModerate = async (voteId: string, action: "validate" | "exclude", notes?: string) => {
    try {
      ...
      const response = await fetch(`/api/admin/arbitrage/arbinote/votes/moderate/${voteId}`, {
        method: "POST",
        headers: {
      ...
      const detailsResponse = await fetch(
        `/api/admin/arbitrage/arbinote/votes/${alert?.match.id}/details`
      );
      if (detailsResponse.ok) {
        const detailsData = await detailsResponse.json();
        setAlert(detailsData);
